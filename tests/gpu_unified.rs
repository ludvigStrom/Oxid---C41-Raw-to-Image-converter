...
fn compare_u16(cpu: &Step6Display, gpu: &Step6Display, tolerance_lsb: u16, label: &str) {
    ...
    if above > 0 {
        panic!(
            ...
            label, above, tolerance_lsb, max_diff,
...
fn compare_f32(cpu: &Step6Display, gpu: &Step6Display, tolerance: f32, label: &str) {
    ...
    if above > 0 {
        panic!(
            ...
            label, above, tolerance, max_diff,
            cpu_img[[max_coords.0, max_coords.1, max_coords.2]],
            gpu_img[[max_coords.0, max_coords.1, max_coords.2]],
        );
    }
...
fn run_cpu_4_5_6(
    ...
    let lut3d = options.lut3d_path.as_ref().and_then(|p| c41_raw_tool::lut3d::read_cube(p).ok());
    ...
    let output_lut = options.output_lut_cube.as_ref().and_then(|p| c41_raw_tool::lut3d::read_cube(p).ok());
...
fn unified_ra4_from_step4() {
    ...
    let gpu_result = gpu.run_from_step(&img, 4, &opts, None, &ra4, None)
...
fn unified_from_step5() {
    ...
    let gpu_result = gpu.run_from_step(&img, 5, &opts, None, &ra4, None)
...
fn unified_from_step6() {
    ...
    let gpu_result = gpu.run_from_step(&img, 6, &opts, None, &ra4, None)
